...
export default function MessageInput({ onSend, onTyping, replyTo, onCancelReply, editingMsg, onCancelEdit, onEditSave, myId }: Props) {
  ...
  const handleVoiceSend = (audioDataUrl: string, duration: number) => {
    setShowVoice(false);
    onSend("", { type: "audio", fileUrl: audioDataUrl, duration });
  };
    ...
    <div className="border-t border-surface-border bg-surface-card flex-shrink-0 safe-area-bottom">
      {replyTo && !isEditing && (
        <div className="flex items-center gap-3 px-4 pt-3 pb-1">
          <div className="flex-1 pl-3 border-l-2 border-brand-500 text-xs text-gray-400 min-w-0">
            <span className="text-brand-500 font-medium">Replying to {replyTo.senderId === myId ? "yourself" : "them"}</span>
            <p className="truncate">{replyTo.type === "image" ? "📷 Image" : replyTo.type === "audio" ? "🎤 Voice" : replyTo.message}</p>
          </div>
          <button onClick={onCancelReply} className="flex-shrink-0 text-gray-600 hover:text-gray-300 p-1">✕</button>
        </div>
      )}
      ...
      {isEditing && (
        <div className="flex items-center gap-3 px-4 pt-3 pb-1">
          ...
          <button onClick={() => { onCancelEdit(); setText(""); }} className="flex-shrink-0 text-gray-600 hover:text-gray-300 p-1">✕</button>
        </div>
      )}
      ...
      <div className="px-3 py-3 relative">
        ...
        <div className="flex items-end gap-2 bg-surface-raised rounded-2xl px-3 py-2 border border-surface-border focus-within:border-brand-500/50 transition-colors">
          ...
          {!isEditing && (
            <>
              <button onClick={() => imageInputRef.current?.click()} className="flex-shrink-0 p-1.5 text-gray-600 hover:text-gray-300 transition-colors rounded-lg" title="Image">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button onClick={() => fileInputRef.current?.click()} className="flex-shrink-0 p-1.5 text-gray-600 hover:text-gray-300 transition-colors rounded-lg" title="File">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <textarea ref={textareaRef} value={text} onChange={handleChange} onKeyDown={handleKeyDown}
            placeholder={isEditing ? "Edit message…" : "Type a message…"} rows={1}
          ...
          {/* Voice button (only when not typing) */}
          {!text.trim() && !isEditing && (
            <button onClick={() => setShowVoice(true)}
              className="flex-shrink-0 p-1.5 text-gray-600 hover:text-brand-500 transition-colors rounded-lg" title="Voice message">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button onClick={handleSend} disabled={!canSend}
            ...
            {uploading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              : isEditing ? <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
