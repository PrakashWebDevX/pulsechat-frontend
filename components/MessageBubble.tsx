      ...
      <div className="relative max-w-[80%] md:max-w-[70%]">
        ...
        {msg.replyTo && (
          <div className={`mb-1 px-3 py-1.5 rounded-lg border-l-2 border-brand-500 bg-surface-raised text-xs text-gray-400 ${isMine ? "ml-auto" : ""}`}>
            ...
            <p className="truncate">{msg.replyTo.type === "image" ? "📷 Image" : msg.replyTo.message}</p>
          </div>
        )}
        ...
        >
          ...
          <div className="flex items-center justify-end gap-1.5 mt-1">
            {msg.edited && (
              <span className={`text-[10px] ${isMine ? "text-brand-100/60" : "text-gray-600"}`}>edited</span>
            )}
            <span className={`text-[10px] ${isMine ? "text-brand-100/70" : "text-gray-600"}`}>
